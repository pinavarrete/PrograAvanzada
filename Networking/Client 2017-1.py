    ...
    def recibir_mensajes(self):
        while self._isalive:
            ...
            if mensaje['status'] == 'msg':
                print("""Mensaje recibido del server: {}
                      """.format(mensaje['content']))

    def enviar(self, mensaje, status):
        if status == 'msg':
            msj_final = {'status': status,
                         'content': self.usuario + ": " + mensaje}
        elif status == 'server_name':
            msj_final = {'status': status,
                         'content': mensaje}
...
if __name__ == '__main__':
    ...
    while True:
        inputing = input('[1] Mandar mensaje\n[2] Desconectarse\n[3]' +
                         'Cambiar nombre Server\n>  ')
        if inputing == '1':
            texto = input('>    ')
            client.enviar(texto, 'msg')
        elif inputing == '2':
            client.desconectar()
            break
        elif inputing == '3':
            texto = input('>    ')
            client.enviar(texto, 'server_name')
